...
1.  The regression coefficients do differ amongst the groups, the intercepts do vary; the catarrhines have a higher baseline ECV (83.2) compared to the platyrrhines (16.18) and the strepsirhines (8.18). I interprety this to mean that the catarrhines have larger brain sizes than the other two groups.
...
# Extract predictor variable (Group_size) and response variable (ECV)
...
# Calculate two-tailed p-value manually
...
#  Generate bootstrap samples and calculate slope coefficients
...
#  Method 1 - Quantile method (percentile bootstrap)
...
#  Method 2 - Theory-based method
# Using the standard deviation as the standard error and t-distribution
df <- original_model$df.residual
t_critical <- qt(0.975, df)
